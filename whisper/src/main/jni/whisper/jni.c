...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_initContext(
    ...
    LOGI("Initializing whisper context from path: %s", model_path_chars);
    struct whisper_context_params cparams = whisper_context_default_params();
    cparams.use_gpu = false;
    cparams.flash_attn = false;
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    const char *lang_chars = lang_str ? (*env)->GetStringUTFChars(env, lang_str, NULL) : NULL;
    const char *target_lang = (lang_chars && strlen(lang_chars) > 0) ? lang_chars : "auto";
    ...
    params.detect_language = false; // NOTE: MUST be false so whisper actually transcribes the audio after detecting language!
    params.n_threads = (num_threads > 0) ? num_threads : 4;
    params.offset_ms = 0;
    params.no_context = false;         // Enable rolling context across chunks for accurate sentences
    params.single_segment = false;
    params.no_timestamps = true;       // Turbo: Disattiva il calcolo dei timestamp sui singoli token per dimezzare i tempi di decodifica
    params.temperature = 0.0f;
    params.temperature_inc = 0.0f;     // Greedy decoding ultra-veloce a singolo passaggio
    params.greedy.best_of = 1;
    params.beam_search.beam_size = 1;
    params.suppress_blank = true;
    params.suppress_nst = false;       // Preserva contrazioni e punteggiatura
    params.max_tokens = 0;

    // Prompt neutro e privo di parole conversazionali per evitare allucinazioni e fraintendimenti lessicali
    if (strcmp(target_lang, "it") == 0) {
        params.initial_prompt = "Trascrizione in lingua italiana, con punteggiatura corretta ed accenti.";
        params.carry_initial_prompt = false;
    } else if (strcmp(target_lang, "en") == 0) {
        params.initial_prompt = "Accurate English transcription with proper punctuation.";
        params.carry_initial_prompt = false;
    } else {
    ...
    LOGI("Running whisper_full with lang='%s', detect_lang=%d, threads=%d, samples=%d",
         target_lang, params.detect_language, params.n_threads, audio_data_length);
    int ret = whisper_full(context, params, audio_data_arr, audio_data_length);
    if (ret != 0) {
        LOGE("Failed to run whisper_full, return code=%d", ret);
    } else {
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_getTextSegment(
    ...
    jstring string = (*env)->NewStringUTF(env, text ? text : "");
    return string;
}
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_getSystemInfo(
    ...
    jstring string = (*env)->NewStringUTF(env, sysinfo ? sysinfo : "");
    return string;
}
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_benchMemcpy(JNIEnv *env, jobject thiz,
    ...
    jstring string = (*env)->NewStringUTF(env, bench_ggml_memcpy ? bench_ggml_memcpy : "");
    return string;
}
...
Java_com_whispercpp_whisper_WhisperLib_00024Companion_benchGgmlMulMat(JNIEnv *env, jobject thiz,
    ...
    jstring string = (*env)->NewStringUTF(env, bench_ggml_mul_mat ? bench_ggml_mul_mat : "");
    return string;
}
